!------------------------------------------------------------------------------
! Compute and assemble first order jacobian in CSR format
! computes by looping over triangles
!------------------------------------------------------------------------------
      ...
      real(dp) :: coord(2,*), tarea(*), dt(*), qc(nvar,*)

      integer  :: it, i, j, p1, p2, p3, t1, t2, t3, irow, &
                  icol, jcount, rcount, iw(nvar*nt), ierr, iwk
      real(dp) :: j0(nvar,nvar), j1(nvar,nvar), j2(nvar,nvar), &
                  j3(nvar,nvar), fact

!     Count number of elements in jmat
      jcount = 0
...
!     Perform two transpositions to order the columns
...
!------------------------------------------------------------------------------
      ...
      implicit none
      include 'common.h'
      real(dp) :: x1(2), x2(2), qc1(*), qc2(*), jac(nvar,nvar)

      integer  :: i
      real(dp) :: r1, u1, v1, p1, a1, r2, u2, v2, p2, a2, &
                  ua, va, aa, una, ct, st, lent, lam
