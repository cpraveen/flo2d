...
!************************************************************************
! On input:
!     res contains adjoint residual
! On output:
!     resb contains incremental adjoint solution
      subroutine agmres(elem, esue, edge, tedge, vedge, spts, &
                        coord, qc, qcb, qv, qvb, qx, qxb, qy, qyb, &
                        af, tarea, varea, dt, cl, cd, res, resb)
      implicit none
      include 'param.h'
      include 'gmres.h'
      ...
      real(dp) :: coord(2,*), qc(nvar,*), af(3,*), qv(nvar,*), tarea(*), &
                  varea(*), res(nvar,*), resb(nvar,*), qcb(nvar,*), dt(*), &
                  qx(3,*), qy(3,*), qxb(3,*), qyb(3,*), cl, cd, &
                  qvb(nvar,*)
...
!        work(colz) <-- A * work(colx)
         ...
         do i=1,nt
            do j=1,nvar
               resb(j,i) = work(colx + icount)
               icount  = icount + 1
            enddo
         enddo
         call fvresidual_bq(elem, edge, tedge, vedge, spts, &
                           coord, qc, qcb, qv, qvb, qx, qxb, qy, qyb, &
                           af, tarea, varea, cl, cd, res, resb)
         do i=1,nt
            call prim2con(qc(1,i), con)
            call con2prim_bq(con, resb(1,i), rdummy, qcb(1,i))
         enddo
         icount = 0
         do i=1,nt
            do j=1,nvar
...
!        call precond(work(colx),work(colz))
         call lutsol(n, work(colx), work(colz), alu, jlu, ju)
         goto 10
