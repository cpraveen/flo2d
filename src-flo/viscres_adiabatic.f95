!------------------------------------------------------------------------------
! For an edge, Add contribution of viscous fluxes to the residual vector
! for adiabatic case
!------------------------------------------------------------------------------
      subroutine viscres_adiabatic(x1, x2, qv1, qv2, qx1, qx2, qy1, qy2, &
                                   res1, res2)
      implicit none
      include 'common.h'
      include 'visc.h'
      real(dp) :: x1(2), x2(2), qv1(nvar), qv2(nvar), qx1(3), &
                  qx2(3), qy1(3), qy2(3), res1(nvar), res2(nvar)

      real(dp) :: u, v, txx1, txx2, tyy1, tyy2,  &
                  txy1, txy2, txx, txy, tyy, ex1, ey1, ex2, ey2, &
                  ex, ey, xmom, ymom, ener, mul1, mul2, mut1, mut2, &
                  mu1, mu2, dx, dy, gp1, gp2, n2b3, sutherland
      external sutherland

      n2b3 = 2.0d0/3.0d0

      mul1 = sutherland(qv1)
      mul2 = sutherland(qv2)

      mu1  = mul1
      mu2  = mul2

      txx1 = n2b3*mu1*(2.0d0*qx1(1) - qy1(2))
      tyy1 = n2b3*mu1*(2.0d0*qy1(2) - qx1(1))
      txy1 =      mu1*(      qy1(1) + qx1(2))

      txx2 = n2b3*mu2*(2.0d0*qx2(1) - qy2(2))
      tyy2 = n2b3*mu2*(2.0d0*qy2(2) - qx2(1))
      txy2 =      mu2*(      qy2(1) + qx2(2))

      txx  = 0.5d0*(txx1 + txx2)
      txy  = 0.5d0*(txy1 + txy2)
      tyy  = 0.5d0*(tyy1 + tyy2)

      dx   = x2(1) - x1(1)
      dy   = x2(2) - x1(2)

      u    = 0.5d0*(qv1(2) + qv2(2))
      v    = 0.5d0*(qv1(3) + qv2(3))

      xmom = txx*dy - txy*dx
      ymom = txy*dy - tyy*dx
      ener = (txx*u + txy*v)*dy - (txy*u + tyy*v)*dx

      res1(2) = res1(2) - xmom
      res1(3) = res1(3) - ymom
      res1(4) = res1(4) - ener

      res2(2) = res2(2) + xmom
      res2(3) = res2(3) + ymom
      res2(4) = res2(4) + ener

      end
