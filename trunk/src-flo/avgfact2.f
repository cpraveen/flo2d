...
C----------------------------------------------------------------------------
      ...
      implicit none
      include 'common.h'
      real(dp) :: x1(2), xp(2), x2(2), nx, ny
      real(dp) :: dx1, dy1, ds1, nx1, ny1,
     +            dx2, dy2, ds2, nx2, ny2,
     +            nxa, nya, nsa
...
C afact(1) and afact(2) must be set to zero before calling this
C function. If det==0 then afact(1)=afact(2)=0. This could happen if
C there is a "corner triangle".
C----------------------------------------------------------------------------
