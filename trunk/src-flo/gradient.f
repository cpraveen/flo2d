...
C------------------------------------------------------------------------------
     ...
     +                    qx, qy)
      implicit none
      include 'param.h'
      integer          elem(3,ntmax), edge(2,nemax), bdedge(2,nbpmax),
     +                 spts(nspmax)
      double precision coord(2,npmax), qc(nvar,ntmax), qv(nvar,npmax)

      integer          i, j, v1, v2, v3, e1, e2
      double precision qx(3,npmax), qy(3,npmax), varea(npmax), const, 
     +                 fact, qtemp(nvar)
...
C------------------------------------------------------------------------------
     ...
     +                   a1, a2, a3, qc)
      implicit none
      include 'common.h'
      double precision x1(2), x2(2), x3(2), qx1(3), qx2(3),
     +                 qx3(3), qy1(3), qy2(3), qy3(3),
     +                 a1, a2, a3, qc(nvar)

      double precision u, v, T, dx1, dx2, dx3, dy1, dy2, dy3, a
...
C------------------------------------------------------------------------------
     ...
     +                 qx2(3), qy1(3), qy2(3)
      ...
      double precision dx, dy, u1, v1, u2, v2, T1, T2, const
